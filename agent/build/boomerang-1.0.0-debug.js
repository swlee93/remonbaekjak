...
(function () {
  ...
  function shouldExcludeXhr(anchor) {
    ...
    return (
      BOOMR.xhr_excludes.hasOwnProperty(anchor.href) ||
  ...
  MutationHandler.pause = function () {
    if (MutationHandler.observer && MutationHandler.observer.observer && !MutationHandler.isPaused) {
        ...
        if (!ev.forced) {
          ...
          if (ev.type === 'spa_hard') {
            ...
            if (
              ...
              ev.resource.timing.loadEventEnd &&
  ...
  MutationHandler.prototype.wait_for_node = function (node, index) {
    ...
    if (
      ...
      url = node.src || (typeof node.getAttribute === 'function' && node.getAttribute('xlink:href')) || node.href;
      ...
      if (isNaN(domHeight)) {
        domHeight =
          node.style && (node.style.height === '0' || node.style.height === '0px' || node.style.height === '1px')
            ? 0
            : undefined;
      }
      ...
      if (isNaN(domWidth)) {
        domWidth =
          node.style && (node.style.width === '0' || node.style.width === '0px' || node.style.width === '1px')
            ? 0
            : undefined;
      }
  ...
  function instrumentFetch() {
    ...
    if (BOOMR.proxy_fetch && BOOMR.proxy_fetch === w.fetch) {
      // already instrumented
      return;
    }

    if (BOOMR.proxy_fetch && BOOMR.orig_fetch && BOOMR.orig_fetch === w.fetch) {
    ...
    BOOMR.proxy_fetch = function (input, init) {
      console.log('proxy_fetch', input, init);
      var url,
        method,
        payload,
      ...
      try {
        resource.timing.requestStart = BOOMR.now();
        console.log('proxy_fetch.arguments', arguments);
                ...
                try {
                  ...
                  res
                    ...
                    .then(null, function (reason) {
                      // `.catch` will cause parse errors in old browsers
  ...
  function uninstrumentFetch() {
    if (typeof w.fetch === 'function' && BOOMR.orig_fetch && BOOMR.orig_fetch !== w.fetch) {
  ...
  function instrumentXHR() {
    if (BOOMR.proxy_XMLHttpRequest && BOOMR.proxy_XMLHttpRequest === w.XMLHttpRequest) {
      // already instrumented
      return;
    }

    if (BOOMR.proxy_XMLHttpRequest && BOOMR.orig_XMLHttpRequest && BOOMR.orig_XMLHttpRequest === w.XMLHttpRequest) {
      // was once instrumented and then uninstrumented, so just reapply the old instrumented object
    ...
    BOOMR.proxy_XMLHttpRequest = function () {
      console.log('proxy_XMLHttpRequest');
      var req,
        resource = { timing: {}, initiator: 'xhr' },
        orig_open,
        orig_send,
      ...
      req.open = function (method, url, async) {
        console.log('proxy_XMLHttpRequest.req.open');
        a.href = url;
      ...
      req.send = function (data) {
        console.log('proxy_XMLHttpRequest.req.send');
        req.setRequestHeader('mtid', 'test');
        if (excluded) {
        ...
        // during .open
        if (typeof resource.status === 'undefined' || resource.status !== XHR_STATUS_OPEN_EXCEPTION) {
          return orig_send.apply(req, arguments);
        }
      };
      ...
      for (idx = 0; idx < impl.excludeFilters.length; idx++) {
        if (typeof impl.excludeFilters[idx].cb === 'function') {
          ...
          try {
            ...
            if (ret) {
              /* BEGIN_DEBUG */
              debugLog('Found matching filter at: ' + impl.excludeFilters[idx].name + ' for URL: ' + anchor.href);
              /* END_DEBUG */
      ...
      if (p && p.timing) {
        ...
        entry = BOOMR.getResourceTiming(
          ...
          function (rt) {
            ...
            return Math.ceil(navSt + rt.startTime + 2) >= resource.timing.requestStart && rt.responseEnd !== 0;
          },
        );
        ...
        if (entry) {
          ...
          if (entryResponseEnd <= BOOMR.now()) {
            // this check could be moved into the fiter above
  ...
  BOOMR.plugins.AutoXHR = {
    ...
    /* BEGIN_DEBUG */ matchesAlwaysSendXhr: matchesAlwaysSendXhr,
    /* END_DEBUG */
  };
